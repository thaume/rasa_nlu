...
class WhysClassifier(Component):

    name = "whys_classifier"
    ...
    def train(self, training_data, config, **kwargs):
        ...
        if len(set(labels)) < 2:
            ...
            X = self.questions_to_x([self.clean_str(example.text) for example in training_data.intent_examples])
            X = np.array(X)
            ...
            ngram_filters = [2, 3, 4]
            nb_feature_maps = [128, 128, 64]
            conv_filters = []

            input_conv = Input(shape=(self.MAX_DOC_LENGTH,))
            emb_seq = embedding(input_conv)

            for i, n_gram in enumerate(ngram_filters):
                conv = Conv1D(activation='relu', input_shape=(self.MAX_DOC_LENGTH, 1), filters=nb_feature_maps[i], kernel_size=n_gram, padding='causal')(emb_seq)
                pool = MaxPooling1D(5)(conv)
                pool = Dropout(0.5)(pool)
                av_pool = Flatten()(pool)
                conv_filters.append(av_pool)

            output_conv = concatenate(conv_filters)

            output_conv = Dropout(0.3)(output_conv)

            x = Dense(y.shape[0], activation='softmax')(output_conv)

            self.model = Model(inputs=input_conv, outputs=x)
